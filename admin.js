 ...
 * New task creation: Platform → TaskType → Title → Link → Reward
 */
...
async function handleAdminSessionInput(ctx, next) {
  ...
  if (s.step === 'task_reward') {
    ...
    const typeLabel = TASK_TYPE_LABELS[s.taskType]||s.taskType;
    const platEmoji = s.platform==='telegram'?'✈️':'🐦';
    const emoji     = s.taskKind==='raid'?'⚡':'🎯';
    const btnLabel  = `${platEmoji} ${typeLabel}`;
    const task = store.createTask(groupId, s.title, s.link, reward, s.taskKind, btnLabel, s.platform, s.taskType);
    const broadcastMsg =
      `${emoji} <b>New ${s.taskKind==='raid'?'Raid':'Task'}!</b>\n${'─'.repeat(28)}\n`+
      `📌 <b>${task.title}</b>\n🏷 ${platEmoji} ${typeLabel}\n`+
      (task.link?`🔗 ${task.link}\n`:'')+
    ...
    if (topicId && group?.topics?.notifications && topicId!==group.topics.notifications) {
      try {
        await ctx.telegram.sendMessage(groupId, broadcastMsg, {
          parse_mode:'HTML', message_thread_id:group.topics.notifications,
          ...taskCardKeyboard(task.id, task.link, btnLabel, task.taskType),
        });
      } catch {}
    }
    ...
    for (const u of users) {
      try { await ctx.telegram.sendMessage(u.id, broadcastMsg, { parse_mode:'HTML', ...taskCardKeyboard(task.id, task.link, btnLabel, task.taskType) }); dmSent++; } catch {}
      await delay(50);
    }
    await ctx.replyWithHTML(`✅ <b>${s.taskKind==='raid'?'Raid':'Task'} created!</b>\n🆔 <code>${task.id}</code>  •  📨 DMs: <b>${dmSent}</b>`);
  ...
  if (s.step==='ban_id')      { session.clearSession(userId); const ok=store.banUser(text.replace('@','')); await ctx.replyWithHTML(ok?`🚫 Banned <code>${text}</code>.`:`⚠️ User not found.`); return sendAdminPanel(ctx, groupId); }
  if (s.step==='unban_id')    { session.clearSession(userId); const ok=store.unbanUser(text.replace('@','')); await ctx.replyWithHTML(ok?`✅ Unbanned <code>${text}</code>.`:`⚠️ User not found.`); return sendAdminPanel(ctx, groupId); }
  ...
  if (s.step==='add_email') {
    session.clearSession(userId);
    if (!text.includes('@')||!text.includes('.')) return ctx.reply('❌ Invalid email.');
...
function register(bot) {
  ...
  bot.action(/^admin_platform_(task|raid)_(twitter|telegram)$/, async (ctx) => {
    ...
    if (!s) return ctx.reply('⚠️ Session expired. Run /admin again.');
    session.setSession(ctx.from.id, { ...s, platform, step:'select_tasktype' });
    const platLabel = platform==='telegram'?'✈️ Telegram':'🐦 Twitter/X';
    await ctx.replyWithHTML(`${platLabel} selected.\n\n<b>Step 2 / 5</b> — Select the <b>task type</b>:`, taskTypeKeyboard(kind, platform));
  });
  ...
  bot.action('admin_view_tasks', async (ctx) => {
    await ctx.answerCbQuery();
    const groupId=resolveAdminGroup(ctx); if (!groupId) return ctx.reply('⚠️ No group selected.');
    ...
    const lines=tasks.map(t=>{
      ...
      const pe=t.platform==='telegram'?'✈️':'🐦';
      return `${t.active?'🟢':'🔴'} [<code>${t.id}</code>] ${t.type==='raid'?'⚡':'🎯'} <b>${t.title}</b>\n   ${pe} ${tl} — ${t.reward}pts`;
    }).join('\n\n');
  ...
  bot.action('admin_announce', async (ctx) => { await ctx.answerCbQuery(); startFlow(ctx, { step:'announce_msg' }, `📣 <b>Announce</b>\n\nType your announcement:`); });
  bot.action('admin_dm_all',   async (ctx) => { await ctx.answerCbQuery(); startFlow(ctx, { step:'dm_all_msg'   }, `📨 <b>DM All Users</b>\n\nType the message:`); });
  ...
  bot.action('admin_view_users', async (ctx) => {
    ...
    const lines=users.map((u,i)=>`${i+1}. @${u.username} (<code>${u.id}</code>) — ${u.points}pts ${u.banned?'🚫':'✅'}`).join('\n');
