...
function requireGroup(ctx) {
  ...
  if (chatType !== 'group' && chatType !== 'supergroup') {
    ctx.reply('⚠️ This command must be run inside the group.');
    return null;
  }
...
async function handleSetTopic(ctx) {
  const groupId = requireGroup(ctx);
  if (!groupId) return;
  if (!store.isGroupRegistered(groupId)) return ctx.reply('⚠️ Group not registered. An owner must /addgroup first.');
...
async function handlePostWelcome(ctx) {
  ...
  const welcomeMsg =
    ...
    `1️⃣ Start the bot in DM: @${botInfo.username}\n` +
    `2️⃣ Link your Twitter via ⚙️ Settings\n` +
    `3️⃣ Complete 🎯 Tasks & ⚡ Raids to earn points\n` +
    `4️⃣ Check the 🏆 Leaderboard for your rank\n\n` +
    `📋 <b>Channels:</b>\n` +
...
async function handleStats(ctx) {
  ...
  if (!groupId || !store.isGroupRegistered(groupId)) {
    return ctx.reply('⚠️ Group not registered or not selected. Run /admin first.');
  }
...
async function handleAddAdmin(ctx) {
  ...
  if (!store.isGroupRegistered(groupId)) return ctx.reply('⚠️ Group not registered or invalid ID.');
