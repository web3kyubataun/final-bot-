/**
 * group.js — Group setup commands: topics, welcome, stats.
 */
...
const TOPIC_TYPES = {
  ...
  tasks:         { label: 'Tasks',            desc: 'Active tasks list' },
...
async function handleListTopics(ctx) {
  ...
  const lines = Object.entries(TOPIC_TYPES).map(([type, info]) => {
    ...
    return `${id ? 'Set' : 'Not set'} ${info.label}: ${id ? `<code>${id}</code>` : '<i>not configured</i>'}`;
  });
...
async function handlePostWelcome(ctx) {
  ...
  const welcomeMsg =
    ...
    `1. Start the bot in DM: @${botInfo.username}\n` +
    `2. Link your Twitter via Settings\n` +
    `3. Complete Tasks and Raids to earn points\n` +
    `4. Check the Leaderboard for your rank\n\n` +
    `<b>Channels:</b>\n` +
...
async function handleStats(ctx) {
  ...
  const group = store.getGroup(groupId);
  const topicsSet = Object.entries(group?.topics || {}).filter(([,v]) => v).length;
  ...
  await ctx.replyWithHTML(
    ...
    `Topics configured: <b>${topicsSet}</b> / ${Object.keys(TOPIC_TYPES).length}`
  );
}
